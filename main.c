...
void Read_Sensor() /* 센서 값을 읽어오는 함수 */
{
	for(int i = 0 ; i < ArrayX - 1 ; i++)
	{
		...
		{
			  GPIOC -> ODR = (1 << j);
			  memcpy(&user_buff[i][j], &adc_buff[0], 1);
		}
	}
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	double Weight[ArrayX][ArrayY] = {{4.74695e-07, 1.49394e-05, 1.03572e-05},
							{4.32258e-06, -2.35728e-06, 1.30236e-06},
							{-4.38062e-06, -3.14077e-06, -4.67674e-06}};
  ...
  while (1)
  {
	 ...
	 if(predict(Weight,user_buff)<0.5)
	 {
		 fact = 0;
	 }else
	 {
